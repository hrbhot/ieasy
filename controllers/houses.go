...
//housest?title=555&p=1
func (this *HousesController) Title() {
	...
	beego.Debug(title)
	beego.Info("打印title浏览器的值=", title)
	houses := []models.House{}

	o := orm.NewOrm()

	qs := o.QueryTable("house")
	//这是原来的注释，注释掉了user_id, 显示id大于1的所有houses内容，双下划线__gt是大于的意思
	num, err := qs.Filter("title", title).All(&houses)

	//      num, err := qs.Filter("area_id", aid).All(&houses)
	beego.Info("0000000000000000000====")
	if err != nil {
	...
	for _, house := range houses {
		...
		//增加title
		//      o.LoadRelated(&house, "title")

		house_list = append(house_list, house.To_house_info())
		beego.Info("打印house_list=", house_list)
	}
...
func (this *HousesController) GetOneHouseInfo() {
	...
	redis_config_map := map[string]string{
		...
		"conn":  ":6379",
		"dbNum": "1",
		//	cache_conn, err := cache.NewCache("redis", `{"key":"ieasy","conn":":6379","dbNum":"0"}`)
